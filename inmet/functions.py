...
def bh(estacao):
    # Suponha que a função 'get_dados' recupere os dados como uma string JSON

    # Get the current date
    current_date = datetime.now()

    # Generate date ranges for each completed month up to the current month
    meses = []
    for i in range(1, current_date.month):
        # Calculate the first day of the month
        first_day = datetime(current_date.year, i, 1).strftime('%Y-%m-%d')
        
        # Calculate the last day of the month
        last_day = (datetime(current_date.year, i + 1, 1) - timedelta(days=1)).strftime('%Y-%m-%d')
        
        # Append the date range to the meses list
        meses.append((first_day, last_day))
    ...
    aux = 100
    aux2 = 0
    # Inicialize um dicionário para armazenar os resultados mensais
    resultados_mensais = {}

    # Itere sobre os intervalos de datas para cada mês
    for data_inicial, data_final in meses:
        # Calcule a evapotranspiração para o mês atual
        resultado_mensal = evapo(data_inicial, data_final, estacao)
        
        # Armazene o resultado no dicionário usando o período como chave
        resultados_mensais[data_inicial] = resultado_mensal

    # Calcule o somatório dos valores mensais de evapotranspiração
    results_json_format = []
    somatorio_mensal = {}
    neg_acum = {}
    NAC = {}
    ...
    # Crie um dicionário para armazenar os valores de EXC para cada mês em português
    meses_exc_ptbr = {}

    # Mapeie os nomes dos meses em português
    nomes_meses = {
    ...
    for data_inicial, resultado in resultados_mensais.items():
        if resultado:
            # Realize o parsing do JSON para acessar a evapotranspiração
            evapotranspiracao = json.loads(resultado)
            # Calcule o somatório da evapotranspiração para o mês
            ETO[data_inicial] = round(sum(entry['ET'] for entry in evapotranspiracao))
            P[data_inicial] = round(sum(entry['CHUVA'] for entry in evapotranspiracao))
            somatorio_mensal[data_inicial] = round(P[data_inicial] - ETO[data_inicial],2)
            neg_acum[data_inicial] = 100 * math.log(somatorio_mensal[data_inicial] / 100) if somatorio_mensal[data_inicial] > 0 else (somatorio_mensal[data_inicial] + neg_acum_anterior) if somatorio_mensal[data_inicial] < 0 else neg_acum_anterior
            
            # Converte a data para o formato desejado (apenas o nome do mês em português)
            data_inicial_obj = datetime.strptime(data_inicial, '%Y-%m-%d')
            mes_numero = data_inicial_obj.month
            nome_mes_ptbr = nomes_meses[mes_numero]

            if somatorio_mensal[data_inicial] <= 0:
                NAC[data_inicial] = round(aux2 + somatorio_mensal[data_inicial],0)
                ARM[data_inicial] = round(CAD * (math.exp(NAC[data_inicial]/CAD)),0)
                ALT[data_inicial] = round(ARM[data_inicial] - aux,0) 
                ETR[data_inicial] = round(P[data_inicial] + abs(ALT[data_inicial]),0)
                aux2 += somatorio_mensal[data_inicial]
            else:
                if all(i == 0 for i in NAC.values()):
                    ARM[data_inicial] = 100
                    NAC[data_inicial] = 0
                    ALT[data_inicial] = round(ARM[data_inicial] - aux,0)
                    ETR[data_inicial] = round(ETO[data_inicial],0)
                    aux2 = 0          
                elif 0 in NAC.values() and somatorio_mensal[data_inicial] > 100:
                    ARM[data_inicial] = 100
                    NAC[data_inicial] = 0
                    ALT[data_inicial] = round(ARM[data_inicial] - aux,0)
                    ETR[data_inicial] = round(ETO[data_inicial],0)
                    ...
                    ARM[data_inicial] = round(aux + somatorio_mensal[data_inicial])
                    NAC[data_inicial] = round(CAD * (math.log(ARM[data_inicial]/ CAD)))
                    ALT[data_inicial] = round(ARM[data_inicial] - aux)
                    ETR[data_inicial] = round(ETO[data_inicial])
                    aux2 = 0 
            aux = round(ARM[data_inicial])
            DEF[data_inicial] = round(ETO[data_inicial] - ETR[data_inicial])
            if ARM[data_inicial] < CAD:
                EXC[data_inicial] = 0
            elif ARM[data_inicial] == CAD:
                EXC[data_inicial] = somatorio_mensal[data_inicial]             
            else:
                EXC[data_inicial] = somatorio_mensal[data_inicial] - ALT[data_inicial] 
            result_entry = {
                "DT_MEDICAO": nome_mes_ptbr, 
                "ETO": ETO[data_inicial],
                "EXC": EXC[data_inicial],
                "DEF": DEF[data_inicial],
                "P": P[data_inicial],
            }

            # Append the dictionary to the list
            results_json_format.append(result_entry)

    # Convert the list of results to JSON
    json_output = json.dumps(results_json_format, indent=2, ensure_ascii=False)
